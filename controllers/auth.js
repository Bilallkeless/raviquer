...
exports.post_register = async function (req, res, next) {
  ...
  try {
    ...
    req.session.message = {
      ...
      class: "text-white p-4 rounded-lg shadow-md mb-4 bg-green-500",
    };
    ...
    if (
      ...
      return res.render("auth/register", {
        ...
        message: {
          text: msg,
          class: "text-white p-4 rounded-lg shadow-md mb-4 bg-red-500",
        },
      });
    } else {
      next(err);
...
exports.post_login = async function (req, res, next) {
  ...
  try {
    ...
    if (!user) {
      return res.render("auth/login", {
        title: "login",
        message: { text: "Email hatalı.", class: "bg-red-500 mb-3" },
      });
    }
    ...
    return res.render("auth/login", {
      title: "login",
      message: { text: "Parola hatalı.", class: "bg-red-500 mb-3" },
    });
  } catch (err) {
    next(err);
  }
...
exports.post_reset = async function (req, res, next) {
  ...
  try {
    ...
    if (!user) {
      req.session.message = { text: "Email bulunamadı", class: "bg-red-500" };
